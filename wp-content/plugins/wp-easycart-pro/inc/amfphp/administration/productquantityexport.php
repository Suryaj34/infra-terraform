...
if( !empty( $users ) ){
	...
	$sql = "SELECT ec_product.product_id, ec_product.model_number, ec_product.stock_quantity FROM ec_product ORDER BY ec_product.product_id ASC LIMIT %d, 1000";
	$results = $wpdb->get_results( $wpdb->prepare( $sql, ( $setnum-1 )*1000 ), ARRAY_A );
	...
	}else if( $total > 1000 ){ // Combine and zip generate files
		if( !class_exists( 'ZipArchive' ) ){
			echo "You are missing the PHP Class ZipArchive. When you have over 1000 products the PHP Class ZipArchive is required to export products. Please contact your hosting provider to have this installed and this problem corrected.";
			die( );
		}else{
			file_put_contents( "productexport" . $setnum . ".csv", $data );
			$files = array( );
